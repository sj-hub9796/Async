...
public class ExecutorManager {
    ...
    private final TickThread[] workerThreads;
    ...
    public ExecutorManager(int workerThreadCount, Consumer<Thread> threadInitializer, int priorityCount) {
        ...
        workerThreads = new TickThread[workerThreadCount];
        for (int i = 0; i < workerThreadCount; i++) {
            final TickThread thread = new TickThread(this);
    ...
    public void shutdown() {
        for (TickThread workerThread : workerThreads) {
    ...
    public void schedule(Runnable runnable, int priority) {
        this.schedule(new TickTask(runnable, priority));
    }
